...
export default function HomePage() {
  ...
  const filteredMovies = useMemo(() => {
    ...
    if (voiceoverOnly) filtered = filtered.filter(m => m.hasVoiceover);
    if (selectedGenre !== 'all') filtered = filtered.filter(m => m.genre.includes(selectedGenre));
    if (searchQuery) {
    ...
    if (activeCategory === 'Мій список') filtered = filtered.filter(m => favorites.includes(m.id));
    if (activeCategory === 'Популярне') filtered = filtered.filter(m => m.isTrending);
    if (activeCategory === 'Переглянуті') {
      const historyIds = continueWatching.map(cw => cw.movieId);
      filtered = filtered.filter(m => historyIds.includes(m.id));
    }
    ...
    filtered.sort((a, b) => {
      switch (sortBy) {
        ...
        case 'myrating': {
          const ra = userRatings[a.id] ?? 0;
          const rb = userRatings[b.id] ?? 0;
          return rb - ra;
        }
        default: return 0;
      }
    ...
    return filtered;
  }, [voiceoverOnly, selectedGenre, searchQuery, sortBy, favorites, activeCategory, userRatings, continueWatching]);
  ...
  useEffect(() => {
    ...
    const interval = setInterval(() => {
      setSimulatedProgress(prev => {
        ...
        updateProgress(selectedMovie.id, Math.round(next));
        return next;
      });
    }, 500);
  ...
  const handleShare = useCallback(async (movie: typeof moviesData[0]) => {
    const shareData = {
    ...
    return (
      <div className="min-h-screen bg-black flex items-center justify-center">
        <div className="text-center space-y-4">
          <div className="bg-gradient-to-br from-kino-yellow-400 via-kino-yellow-500 to-kino-yellow-600 p-3 rounded-lg animate-pulse inline-block">
          ...
          <div className="flex justify-center space-x-2">
            {[0, 150, 300].map((delay) => (
              <div key={delay} className="w-2 h-2 bg-kino-yellow-400 rounded-full animate-bounce" style={{ animationDelay: `${delay}ms` }} />
            ))}
          </div>
        </div>
      </div>
    ...
    <div className="min-h-screen bg-black text-white">
        ...
        <div className="max-w-7xl mx-auto px-4 md:px-6 py-3 md:py-4">
          <div className="flex items-center justify-between">
            <div className="flex items-center space-x-4 md:space-x-8">
              {/* Logo */}
              <div className="flex items-center space-x-2 group cursor-pointer" onClick={() => { setActiveCategory('Головна'); resetFilters(); }}>
                ...
                <span className="text-xl md:text-2xl font-black font-bebas tracking-tight gradient-text">КІНО.UA</span>
              </div>
            ...
            <div className="flex items-center space-x-2 md:space-x-4">
              ...
              <div className="relative">
                <div className={`flex items-center bg-white/10 backdrop-blur-md rounded-full px-3 md:px-4 py-1.5 md:py-2 transition-all duration-300 border ${
                  ...
                  />
                  {searchQuery && (
                    <button onClick={() => setSearchQuery('')} className="ml-2 text-gray-400 hover:text-white transition-colors">
                      <X className="w-4 h-4" />
                    </button>
                  )}
                </div>
                ...
                {searchQuery && isSearchFocused && searchSuggestions.length > 0 && (
                  <div className="absolute top-full mt-2 w-full md:w-96 bg-gray-900/98 backdrop-blur-xl rounded-xl border border-gray-800 shadow-2xl overflow-hidden animate-fadeIn z-50">
                    ...
                    {searchSuggestions.map(movie => (
                      <div
                        key={movie.id}
                        onClick={() => { handleMovieClick(movie); setSearchQuery(''); }}
                        className="p-3 hover:bg-gray-800/50 cursor-pointer transition-all flex items-center space-x-3 group"
                      >
                        <div
                          className="w-12 h-16 rounded overflow-hidden flex-shrink-0 group-hover:scale-105 transition-transform"
                        ...
                        <div className="flex-1 min-w-0">
                          <div className="text-sm font-semibold text-white group-hover:text-kino-yellow-400 transition-colors truncate">{movie.title}</div>
      ...
      <div className="relative h-screen overflow-hidden">
        <div
          ...
          style={{ background: currentHero.backdrop, transform: `scale(${1 + currentSlide * 0.02})` }}
        >
          ...
          <div className="max-w-7xl mx-auto px-4 md:px-6 w-full">
            <div className="max-w-2xl space-y-4 md:space-y-6 animate-fadeInUp">
              ...
              <div className="flex flex-wrap items-center gap-3 md:gap-4 text-xs md:text-sm text-gray-300">
                ...
                <span>•</span>
                <span>{currentHero.genre.join(', ')}</span>
              </div>
        ...
        {/* Arrows */}
        <button onClick={prevSlide} className="absolute left-2 md:left-6 top-1/2 -translate-y-1/2 glass border p-2 md:p-4 rounded-full hover:bg-black/60 transition-all duration-200 group z-10">
          <ChevronLeft className="w-5 h-5 md:w-6 md:h-6 group-hover:-translate-x-1 transition-transform" />
        </button>
        <button onClick={nextSlide} className="absolute right-2 md:right-6 top-1/2 -translate-y-1/2 glass border p-2 md:p-4 rounded-full hover:bg-black/60 transition-all duration-200 group z-10">
          ...
          <div className="flex flex-wrap items-center justify-between gap-3 md:gap-4">
            ...
            <div className="flex items-center space-x-2">
              ...
              >
                ...
                <option value="myrating">За моєю оцінкою</option>
              </select>
            </div>

            <div className="text-sm text-gray-400">
              Знайдено: <span className="font-bold text-kino-yellow-400">{filteredMovies.length}</span>{' '}
              {filteredMovies.length === 1 ? 'фільм' : 'фільмів'}
            </div>
        ...
        <div className="max-w-7xl mx-auto px-4 md:px-6">
          <div className="mb-6 md:mb-8">
            <h2 className="text-2xl md:text-3xl font-black font-bebas mb-2 gradient-text">
              {activeCategory === 'Мій список' ? 'Обране' : activeCategory === 'Переглянуті' ? 'Переглянуті фільми' : activeCategory}
            </h2>
          ...
          {filteredMovies.length === 0 ? (
            <div className="text-center py-16 md:py-20">
              <Film className="w-16 h-16 md:w-20 md:h-20 mx-auto opacity-20 text-gray-500 mb-4" />
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-3 md:gap-6">
              {filteredMovies.map((movie, idx) => {
                ...
                const cwEntry = continueWatching.find(cw => cw.movieId === movie.id);
                return (
                  <div
                    key={movie.id}
                  ...
                  >
                    <div className="relative overflow-hidden rounded-xl aspect-[2/3] mb-3 border border-gray-800 group-hover:border-kino-yellow-400/50 transition-all duration-300">
                      ...
                      {/* My star rating badge */}
                      {myRating && (
                        <div className="absolute top-2 left-14 bg-purple-500/80 backdrop-blur-sm px-2 py-1 rounded-lg flex items-center space-x-1">
                          <Star className="w-3 h-3 text-white fill-white" />
                          <span className="text-xs font-bold text-white">{myRating}</span>
                        </div>
                      )}
                      ...
                      {/* Progress bar */}
                      {cwEntry && cwEntry.progress > 0 && (
                        <div className="absolute bottom-0 left-0 right-0 h-1 bg-gray-700">
                          <div
                            className="h-full bg-kino-yellow-400"
                            style={{ width: `${cwEntry.progress}%` }}
                          />
                        </div>
                      )}
                    </div>
          ...
          <div className="bg-kino-dark-800 rounded-2xl max-w-4xl w-full max-h-[90vh] overflow-y-auto border border-gray-800 shadow-2xl animate-scaleIn">
            ...
            <div className="relative h-64 md:h-80 overflow-hidden rounded-t-2xl">
              ...
              <div className="absolute bottom-0 left-0 right-0 p-6 md:p-8">
                <h2 className="text-3xl md:text-5xl font-black font-bebas mb-2 gradient-text drop-shadow-lg">
                  {selectedMovie.title}
                </h2>
            ...
            <div className="p-6 md:p-8 space-y-6">
              ...
              {isPlaying ? (
                <div className="relative aspect-video bg-black rounded-xl overflow-hidden border border-gray-800">
                  <div className="absolute inset-0 flex items-center justify-center">
                    <div className="text-center space-y-4 w-full px-8">
                      ...
                      <p className="text-gray-400 font-semibold">🎬 Відеоплеєр (iframe / HLS.js)</p>
                      <p className="text-sm text-gray-500">Інтеграція VideoCDN / Ashdi API</p>
                      {/* Fake progress bar */}
                      <div className="w-full bg-gray-800 rounded-full h-2 mt-4">
                        <div
                          className="bg-kino-yellow-400 h-2 rounded-full transition-all duration-500"
                ...
                >
                  <div className="text-center">
                    ...
                    {continueWatching.find(cw => cw.movieId === selectedMovie.id) && (
                      <p className="text-xs text-kino-yellow-400 mt-1">
                        ▶ Продовжити з {continueWatching.find(cw => cw.movieId === selectedMovie.id)?.progress}%
                      </p>
                    )}
                  </div>
                </button>
                ...
                <div className="flex flex-wrap gap-2">
                  {selectedMovie.genre.map(g => (
                    <button
                      ...
                      className="bg-white/10 hover:bg-kino-yellow-400/20 hover:border-kino-yellow-400/30 px-3 py-1 rounded-full text-sm text-gray-300 hover:text-kino-yellow-400 border border-white/20 transition-all"
                    >
                      {g}
                    </button>
                  ))}
              ...
              <div className="pt-2 border-t border-gray-800/50">
                <UserRating movieId={selectedMovie.id} />
              </div>

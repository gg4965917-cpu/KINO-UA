...
interface MovieStore {
  ...
  setSortBy: (sort: 'rating' | 'year' | 'title' | 'trending' | 'myrating') => void;
  ...
  persist(
    (set, get) => ({
      ...
      setUserRating: (movieId, rating) => {
        ...
        get().addNotification(`Ваша оцінка: ${'⭐'.repeat(rating)}`, 'success');
      },
      ...
      removeFromContinueWatching: (movieId) => set((state) => ({
        continueWatching: state.continueWatching.filter(cw => cw.movieId !== movieId)
      })),
